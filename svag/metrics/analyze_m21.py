...
def compute_jga(dic, mwz_ver, args):
    with open("../data/MWZProcessor/data/{}/ontology.json".format(mwz_ver), encoding="utf-8") as f:
        ontology = json.load(f)
    slots = get_slot_information(ontology)
    with open("../data/MWZProcessor/data/{}/test_dials.json".format(mwz_ver), encoding="utf-8") as f:
        data = json.load(f)
    j_acc = 0
    turns = 0
    err_lst = []
    for each in data:
        ...
        for turn in each["dialogue"]:
            turns += 1
            ground_truth = set(s + '==' + v.replace(' ', '') for s, v in turn["belief_state"].items())
            ...
            last_belief_state = fix_general_label_error([{"slots": [[k, v]]} for k, v in last_belief_state.items()],
                                                        False, slots)
            predict = set(s + '==' + v.replace(' ', '') for s, v in last_belief_state.items())
...
def update_state(state, turn_label):
    for s, v in turn_label.items():
        ...
        # d, ss = slot.split('-')
        # if value in state.values() and ss == 'name':
        #     to_be_deleted = ''
        #     for a, b in state.items():
        #         if b == value:
        #             to_be_deleted = a
        #             break
        #     del state[to_be_deleted]
        if value == "none":
            try:
                del state[slot]
            except:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--remove_blank', default=0, type=int)
    args = parser.parse_args()
    # analyze(args.result_path)
    # compute_jga(get_turn_label(args.result_path), "mwz2_1", args)
    for file_name in os.listdir('../T5DST/output'):
        print(file_name)
        compute_jga(get_turn_label('../T5DST/output/{}'.format(file_name)), "mwz2_1", args)
